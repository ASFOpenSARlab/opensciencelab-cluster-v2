...
class OidcProviderStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        cdk_deploy_roles = {
            ...
            "deploy_cluster_stack_role": {
                "description": "IAM Role for Cluster Deployment from GitHub Actions",
                "name": "AllowUpdateClusterStack",
                "statements": [
                    {
                        "effect": "allow",
                        "actions": ["cloudformation:*"],
                        "resources": [
                            f"arn:aws:cloudformation:{self.region}:{self.account}:stack/*cluster*/*",
                        ],
                    },
                    {
                        "effect": "allow",
                        "actions": ["cloudformation:*"],
                        "resources": [
                            f"arn:aws:cloudformation:{self.region}:{self.account}:stack/ClusterCdkStack-*/*",
                        ],
                    },
                    {  # Allow self modify
                        "effect": "allow",
